        ...
        try:
            ...
            if existing_order:
                ...
                if existing_order.status == 'pending' and age_seconds < 90:
                    ...
                    if txn:
                        response_data = OrderResponseSerializer(existing_order).data
                        response_data['checkout_request_id'] = txn.checkout_request_id
                        response_data['resuming'] = True
                ...
                if existing_order.status == 'pending' and age_seconds >= 90:
                    ...
                    Event.objects.filter(id=event_id).update(
                        available_tickets=F('available_tickets') + existing_order.quantity
                    )

            # Create new order inside atomic transaction
            with transaction.atomic():
                # Lock event row and check tickets
                ...
                # Calculate pricing (Model B)
                subtotal = event.price * quantity
                platform_fee = (subtotal * fee_percentage).quantize(Decimal('0.01'))
                total = subtotal  # Buyer pays subtotal only

                # Generate idempotency key
                now = timezone.now()
                idem_data = f"{user.id}:{event_id}:{quantity}:{now.isoformat()}"
            ...
            try:
                ...
                # Rollback: mark order failed and release tickets
                order.status = 'failed'
                order.save()
                Event.objects.filter(id=event_id).update(
                    available_tickets=F('available_tickets') + quantity
            ...
            response_data['checkout_request_id'] = daraja_response['CheckoutRequestID']
            response_data['resuming'] = False
