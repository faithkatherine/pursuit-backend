    ...
    def handle(self, *args, **options):
        ...
        faith_payment_config, created = OrganizerPaymentConfig.objects.get_or_create(
            ...
            defaults={
                ...
                'payout_type': 'b2c',
                'payout_destination': '254790517633',
                'verified': True,
            }
        )
        if created:
        ...
        # 5. Create TicketTiers for events
        self.stdout.write('\n=== Creating Ticket Tiers ===\n')

        # Single tier events
        single_tier_configs = [
            {
                'title_fragment': 'Afro Night',
                'tier': {
        ...
        for config in single_tier_configs:
            event = Event.objects.filter(name__icontains=config['title_fragment']).first()
            if event:
                tier, created = TicketTier.objects.get_or_create(
                    event=event,
                    name=config['tier']['name'],
                    defaults=config['tier']
                )
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Created tier: {config['tier']['name']} for {event.name}"
                        )
                    )
                else:
                    self.stdout.write(
                        f"  Found existing tier: {config['tier']['name']} for {event.name}"
                    )

        # Multi-tier event: Jazz Evening
        jazz = Event.objects.filter(name__icontains='Jazz').first()
        if jazz:
            jazz_tiers = [
                {
                    'name': 'General Lawn',
                    'description': 'Lawn entry from 1:30 PM',
                    'price': Decimal('1500.00'),
                    'capacity': 30,
                    'available': 30,
                    'sort_order': 0,
                ...
                {
                    'name': 'Picnic Lounge',
                    'description': 'Reserved spot, chilled box included',
                    'price': Decimal('3500.00'),
                    'capacity': 10,
                    'available': 10,
                    'sort_order': 1,
            ...
            for tier_config in jazz_tiers:
                tier, created = TicketTier.objects.get_or_create(
                    event=jazz,
                    name=tier_config['name'],
                    defaults=tier_config
                )
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Created tier: {tier_config['name']} for {jazz.name}")
                    )
                else:
                    self.stdout.write(f"  Found existing tier: {tier_config['name']} for {jazz.name}")

        # Multi-tier event: Premium Event
        premium = Event.objects.filter(name__icontains='Premium').first()
        if premium:
            premium_tiers = [
                {
                    'name': 'General Admission',
                ...
                {
                    'name': 'VIP',
                    'description': 'Stage-side standing, free-bar wristband',
            ...
            for tier_config in premium_tiers:
                tier, created = TicketTier.objects.get_or_create(
                    event=premium,
                    name=tier_config['name'],
                    defaults=tier_config
                ...
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Created tier: {tier_config['name']} for {premium.name}")
                    )
                else:
                    self.stdout.write(f"  Found existing tier: {tier_config['name']} for {premium.name}")

        # 6. Create PlatformConfig
        ...
        self.stdout.write(f'Ticket Tiers: {TicketTier.objects.count()} tiers')
