...
class User(AbstractUser):
    ...
    provider_id = models.CharField(
        max_length=255, blank=True, null=True, help_text="ID from the authentication provider"
    )
    ...
    email_verification_token_hash = models.CharField(
        max_length=64, blank=True, null=True, help_text="SHA-256 hash of verification token"
    )
    email_verification_sent_at = models.DateTimeField(blank=True, null=True)
    password_reset_token_hash = models.CharField(
        max_length=64, blank=True, null=True, help_text="SHA-256 hash of reset token"
    )
...
class UserProfile(models.Model):
    ...
    interests = models.ManyToManyField(
        "core.Interest", blank=True, related_name="users", help_text="User's selected interests for personalization"
    )
    ...
    theme = models.CharField(
        max_length=20, choices=(("light", "Light"), ("dark", "Dark"), ("auto", "Auto")), default="auto"
    )
    ...
    def is_premium(self):
        ...
        return self.payment_plan == "premium" and (
            self.subscription_expires_at is None or self.subscription_expires_at > django_timezone.now()
        )
...
class RefreshToken(models.Model):
    ...
    session = models.ForeignKey(
        UserSession, on_delete=models.CASCADE, related_name="refresh_tokens", blank=True, null=True
    )
